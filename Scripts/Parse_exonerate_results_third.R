...
#import found genes coordinates
found_genes <- read.table("Coordinates_already_examined.tsv", header=FALSE, sep="\t")


#rename column of the blast result table and of found genes
...
#Remove genes in already found coordinates
for (row in 1:nrow(found_genes)) {

  exonerate_rslt <- exonerate_rslt %>% filter(!(seqnames == as.character(found_genes[row, "scaffold"]) & start <= (found_genes[row, "start"]) & end >= (found_genes[row, "start"])))
  exonerate_rslt <- exonerate_rslt %>% filter(!(seqnames == as.character(found_genes[row, "scaffold"]) & start >= (found_genes[row, "start"]) & end <= (found_genes[row, "end"])))
  exonerate_rslt <- exonerate_rslt %>% filter(!(seqnames == as.character(found_genes[row, "scaffold"]) & start <= (found_genes[row, "end"]) & end >= (found_genes[row, "end"])))
...
exonerate_rslt_filtered <- as.data.frame(exonerate_rslt_filtered %>% mutate(query_length = query_end - query_start) %>% mutate(scaffold_length = end-start))


#Make filters on gene length and on scaffold length to find best hits
Best_hits_filtered <- as.data.frame(NULL) 

for(putative_protein_length in c( 400, 300, 200, 100)){ 
...
if (nrow(Best_hits_filtered) > 0) {
	...
	Best_hits_filtered <- Best_hits_filtered %>% mutate(extanded_start = start-300) %>% mutate(extanded_end = end+300)
